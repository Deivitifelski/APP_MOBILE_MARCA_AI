...
export default function DespesasEventoScreen() {
  ...
  const [participationSearchResults, setParticipationSearchResults] = useState<
    ArtistaBuscaConvite[]
  >([]);
  const [participationSearchLoading, setParticipationSearchLoading] =
    useState(false);
  const [participationSearchError, setParticipationSearchError] = useState<
    string | null
  >(null);
  ...
  const [selectedArtistToInvite, setSelectedArtistToInvite] = useState<{
    id: string;
    name: string;
  } | null>(null);
  const [sendingParticipationInvite, setSendingParticipationInvite] =
    useState(false);
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      if (expensesResult.success) {
        ...
        Alert.alert(
          "Erro",
          expensesResult.error || "Erro ao carregar despesas",
        );
      }
      ...
      if (eventResult.success && eventResult.event) {
        ...
        const { data: membership } = user
          ? await supabase
              ...
              .select("role")
              .eq("user_id", user.id)
              .eq("artist_id", ev.artist_id)
              .maybeSingle()
          : { data: null };
        const canManage =
          membership?.role === "admin" ||
          (membership?.role === "vendedor" && ev.created_by === user?.id);
        setCanManageExpenses(canManage);
        if (!canManage) {
          Alert.alert(
            "Acesso restrito",
            "Você só pode gerenciar despesas dos eventos que criou.",
    ...
    eventId,
    activeArtist?.id,
    loadParticipationInvites,
    refreshInvitePermission,
  ]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      void (async () => {
        ...
        const { artists, error } = await buscarArtistasParaConvite(
          participationSearch,
          eventData.artist_id,
        );
  ...
  const submitParticipationInvite = async () => {
    ...
    if (
      !inviteCacheDraft.trim() ||
      Number.isNaN(cacheNumerico) ||
      cacheNumerico <= 0
    ) {
      Alert.alert(
        "Cachê obrigatório",
        "Informe um valor de cachê válido para enviar o convite.",
      );
      return;
    }
    if (!funcaoLimpa) {
      Alert.alert(
        "Função obrigatória",
        "Informe a função do participante para enviar o convite.",
      );
      return;
    }
  ...
  const handleDeleteExpense = (expenseId: string, expenseName: string) => {
    if (!canManageExpenses) {
      Alert.alert(
        "Acesso restrito",
        "Você não pode gerenciar despesas deste evento.",
      );
      return;
    }
    Alert.alert(
    ...
    >
      ...
      >
        ...
        >
          ...
          <Text style={[styles.summaryEvent, { color: colors.textSecondary }]}>
            {eventName}
          </Text>
