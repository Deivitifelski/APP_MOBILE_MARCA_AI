...
export interface ArtistInvite {
  ...
  from_user_id: string;
  to_user_id: string;
  role: "viewer" | "vendedor" | "admin"; // Role que será atribuída ao aceitar
...
export interface CreateInviteData {
  ...
  toUserId: string;
  fromUserId: string;
  role?: "viewer" | "vendedor" | "admin"; // Role a ser atribuída (padrão: viewer)
}

export interface InviteResponse {
  success: boolean;
  error?: string;
  invite?: ArtistInvite | null;
  invites?: ArtistInvite[];
}

// Criar convite para colaborar (agora apenas na tabela notifications)
export const createArtistInvite = async (
  ...
  try {
    ...
    if (!slot.ok) {
      return {
        ...
        error:
          slot.userMessage ||
          "Limite do plano gratuito atingido para este artista.",
      };
    }
...
export const getArtistInvitesReceived = async (
  userId: string,
): Promise<InviteResponse> => {
  try {
...
export const getArtistInvitesSent = async (
  userId: string,
): Promise<InviteResponse> => {
  try {
    ...
    const toUserIds = [
      ...new Set((notifications || []).map((n) => n.to_user_id)),
    ];
    ...
    const invites: ArtistInvite[] = (notifications || []).map(
      (notification) => {
        const toUser = usersMap.get(notification.to_user_id);
        const artist = Array.isArray(notification.artist)
          ? notification.artist[0]
          : notification.artist;
        ...
        return {
          ...
          role: (notification.role || "viewer") as
            | "viewer"
            | "vendedor"
            | "admin",
...
export const acceptArtistInvite = async (
  ...
  try {
    ...
    const slot = await assertArtistTeamSlot(
    ...
    if (!slot.ok) {
      return {
        ...
        error:
          slot.userMessage ||
          "Este artista já atingiu o limite de colaboradores no plano gratuito.",
      };
    }

    const roleToUse = normalizeArtistMemberRole(
      String(notification.role || "viewer"),
    );
...
export const declineArtistInvite = async (
  notificationId: string,
  userId: string,
): Promise<InviteResponse> => {
  try {
...
export const markInviteAsRead = async (
  inviteId: string,
  userId: string,
): Promise<InviteResponse> => {
  try {
    ...
    return {
      success: false,
      error: "Erro interno ao marcar convite como lido",
    };
  }
};
...
export const cancelArtistInvite = async (
  notificationId: string,
  fromUserId: string,
): Promise<InviteResponse> => {
  try {
...
export const checkPendingInvite = async (
  ...
  try {
    console.log("🔍 checkPendingInvite: Buscando convite pendente", {
      artistId,
      toUserId,
    });
    ...
    const { data: notifications, error } = await supabase
      .from("notifications")
      .select(
        "id, artist_id, from_user_id, to_user_id, role, status, read, created_at, type",
      )
