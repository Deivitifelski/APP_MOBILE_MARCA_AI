...
export const acceptCollaboratorInvite = async (
  ...
  try {
    ...
    const rawRole = roleMatch ? roleMatch[1].toLowerCase() : "viewer";
...
export const rejectCollaboratorInvite = async (
  notificationId: string,
): Promise<{ success: boolean; error: string | null }> => {
  try {
    // Marcar notificação como lida e atualizar o tipo
...
export const getPendingInvites = async (
  userId: string,
): Promise<{ invites: any[] | null; error: string | null }> => {
  try {
